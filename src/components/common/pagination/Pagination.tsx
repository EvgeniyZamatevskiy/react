...
export const Pagination = ({
  ...
  const [secondDotsDropdownIsOpen, setSecondDotsDropdownState] =
    useState(false);

  const toggleFirstDotsDropdown = useCallback(
    () => setFirstDotsDropdownState((prevState) => !prevState),
    []
  );
  const toggleSecondDotsDropdown = useCallback(
    () => setSecondDotsDropdownState((prevState) => !prevState),
    []
  );
  ...
  return (
    <ul className="flex items-center">
      {currentPage !== 1 && (
        <li
          className={clsx(paginationItemCss, "hidden md:flex md:visible")}
          onClick={onPrevious}
        >
      ...
      {paginationRange.map((pageNumber, index) => {
        ...
        const ref = isFirstDotsDropdown
          ? firstDotsDropdownRef
          : secondDotsDropdownRef;
        ...
        if (pageNumber === DOTS) {
          ...
          const isOpen = isFirstDotsDropdown
            ? firstDotsDropdownIsOpen
            : secondDotsDropdownIsOpen;
          const toggle = isFirstDotsDropdown
            ? toggleFirstDotsDropdown
            : toggleSecondDotsDropdown;
